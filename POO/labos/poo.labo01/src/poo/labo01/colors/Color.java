...
public final class Color {
	...
	public Color rotate(float angleInDegrees) {
    	return Color.fromHSL((int)(getHue()+angleInDegrees)%360, getSaturation(), getLightness());
    }
	...
	public Color withSaturation(float newSaturation) {
		return Color.fromHSL(getHue(), newSaturation, getLightness());
	}
	...
	/**
	 * Returns an HTML Hex representation of this color.
	 * 
	 * The representation uses the format {@code #RRGGBB}
	 * */
	public String toHTMLHex() {
		String redAsString = String.format("%02X", getRed());
		String greenAsString = String.format("%02X", getGreen());
		String blueAsString = String.format("%02X", getBlue());
