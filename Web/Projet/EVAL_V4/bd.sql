...
create table if not exists Q240183.Categorie
(
...
create table if not exists Q240183.Zone
(
...
create table if not exists Q240183.Emplacement
(
    ...
    constraint Emplacement_ibfk_1
        foreign key (zid) references Q240183.Zone (zid)
);

create table if not exists Q240183.Brocanteur
(
    ...
    constraint Brocanteur_ibfk_1
        foreign key (eid) references Q240183.Emplacement (eid)
);

create index idx_zid
    on Q240183.Emplacement (zid);

create table if not exists Q240183.Objet
(
    ...
    constraint Objet_ibfk_1
        foreign key (cid) references Q240183.Categorie (cid),
    constraint Objet_ibfk_2
        foreign key (bid) references Q240183.Brocanteur (bid)
);

create index idx_bid
    on Q240183.Objet (bid);

create index idx_cid
    on Q240183.Objet (cid);

create table if not exists Q240183.query_logs
(
...
-- INSERTS --
-------------
insert into Q240183.Categorie (cid, intitule)
values  (1, 'Drôle'),
        (2, 'Légendaire'),
        (3, 'Mythique'),
        (4, 'Normal'),
...
insert into Q240183.Zone (zid, nom, description)
...
insert into Q240183.Emplacement (eid, code, zid)
...
insert into Q240183.Brocanteur (bid, nom, prenom, courriel, mot_passe, photo, description, visible, est_administrateur, eid, a_paye, isDeleted)
...
insert into Q240183.Objet (oid, intitule, image, description, cid, bid, isDeleted)
